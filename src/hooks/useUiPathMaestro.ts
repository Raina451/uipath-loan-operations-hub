...
/**
 * Fetch variables for a Maestro process instance
 *
 * Returns all global and element-scoped variables for the process instance.
 * Variables can be filtered by parent element ID to get variables for a specific scope.
 * variables are per applicant/instance details 
 *
 * @param instanceId - The unique identifier of the process instance
 * @param folderKey - The folder key where the instance resides
 * @param options - Optional query options including parentElementId filter
 */
export function useUiPathMaestroVariables(
	instanceId: string | undefined,
	folderKey: string | undefined,
	options?: { enabled?: boolean; parentElementId?: string }
): UseQueryResult<ProcessInstanceGetVariablesResponse, Error> {
	return useQuery({
		queryKey: ['uipath', 'maestro', 'variables', instanceId, folderKey, options?.parentElementId],
		queryFn: async (): Promise<ProcessInstanceGetVariablesResponse> => {
			...
			try {
				const queryOptions: ProcessInstanceGetVariablesOptions = {};
				if (options?.parentElementId) {
					queryOptions.parentElementId = options.parentElementId;
				}

				const result = await uipath.maestro.processes.instances.getVariables(
					instanceId,
					folderKey,
